...
function buildBoxes() {
  ...
  const scale = (Number.isFinite(boxScale) ? boxScale : 135) / 100;
...
function applyKnownFixes(arr){
  ...
  // Q17 (index 16): 混み合う -> 込み合う / こん -> こ
  if (arr[16] && arr[16].answerFull === "混み合う駅") {
    arr[16].qRuby = "”こ”み<ruby>合<rt>あ</rt></ruby>う<ruby>駅<rt>えき</rt></ruby>";
    arr[16].answerFull = "込み合う駅";
    arr[16].units = [
      { kind:"blank", expected:"込", reading:"こ" },
      { kind:"fixed", char:"み" },
      { kind:"fixed", char:"合", reading:"あ" },
      { kind:"fixed", char:"う" },
    ];
  }
  // Q19 (index 18): ざいざい -> ざいさん
  if (arr[18] && typeof arr[18].qRuby === "string" && arr[18].qRuby.includes("”ざい”")) {
    // only replace if it looks like the old pattern
    arr[18].qRuby = arr[18].qRuby.replace("”ざい”","”さん”");
  }
  return arr;
}
...
function loadBoxScale(){
  const s = localStorage.getItem(LS_KEYS.boxScale);
  const n = Number(s);
  // default 135%
  if (Number.isFinite(n) && n >= 70 && n <= 240) return n;
  return 135;
}
function saveBoxScale(){
  localStorage.setItem(LS_KEYS.boxScale, String(boxScale));
}
function updateBoxScaleUI(){
  if (boxScaleLabel) boxScaleLabel.textContent = `枠：${boxScale}%`;
}
function changeBoxScale(delta){
  boxScale = Math.max(70, Math.min(240, boxScale + delta));
  saveBoxScale();
  updateBoxScaleUI();
  redraw();
}
